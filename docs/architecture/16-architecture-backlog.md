...
| ADR relacionado | Nenhum (a criar em ARQ-701) — decisão de nomenclatura (sub-extensão `--ad-pillar-*`) documentada apenas como comentário em `tokens.css`; candidata a virar ADR curto quando ARQ-701 formalizar o processo |
...
| ADR relacionado | Nenhum (a criar em ARQ-701) — decisão técnica (tokens `--breakpoint-*` como constantes documentadas + guarda automatizada, não `var()` funcional dentro de `@media`) documentada como comentário em `tokens.css` e nestas Observações; mesma disciplina de ARQ-301, candidata a virar ADR curto quando ARQ-701 formalizar o processo |
...
| ADR relacionado | Nenhum (a criar em ARQ-701) — decisão de convenção (contador incremental único + guard-test de hash, não hash de conteúdo automatizado nem data única) documentada nestas Observações e como comentário em `tests/cache-busting.spec.ts`/`tests/support/asset-versions.js`; mesma disciplina de ARQ-301/ARQ-302, candidata a virar ADR curto quando ARQ-701 formalizar o processo |
