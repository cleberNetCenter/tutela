...
| Estimativa | M-G |
| Responsável | Segurança |
| Status | **CONCLUÍDO (produção) — 17/08/2026**. Após o período de observação em `Content-Security-Policy-Report-Only` (ativo desde a Sprint 38, 2026-08-05, sem nenhuma violação registrada), o header foi promovido para modo bloqueante (`Content-Security-Policy`, sem sufixo) em **produção** — mesma política, só o nome do header mudou. Aplicado manualmente pelo usuário em `/etc/nginx/sites-enabled/tutela.conf` (host `tutela-web`, fora deste repositório, mesmo padrão de todas as intervenções de Nginx documentadas), validado via inspeção DevTools (Console + Network) em 3 páginas de maior risco (home, `/diagnostico/`, `/insights/ativos-digitais/`) e confirmado via `curl -sI` externo — ver `docs/ambientes-e-deploy.md`, seção "CSP promovido para modo bloqueante em produção (ARQ-102) — 17/08/2026". Critério de aceite (header `Content-Security-Policy` em 100% das respostas do site público, sem quebrar GA/GTM/reCAPTCHA/fontes) satisfeito para produção. **Escopo residual, não bloqueante**: homologação segue em `Content-Security-Policy-Report-Only` — o diff aplicado está restrito a `server_name www.tuteladigital.com.br`; estender o modo bloqueante a homologação fica para sprint futura, por não fazer parte do critério de aceite original (site público). Histórico anterior (Blocos A/B/C, Sprint 38) preservado nas Observações abaixo. |
