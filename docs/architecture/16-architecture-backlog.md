...
| Estimativa | M-G |
| Responsável | Segurança |
| Status | BLOCO A/B/C CONCLUÍDOS (Sprint 38, 2026-08-05). Confirmado por auditoria via `curl -sS -D -` externo (sem SSH — ambiente segue sem acesso aos servidores): o header `Content-Security-Policy-Report-Only` com o texto exato desenhado no Bloco B está ATIVO em **ambos** os ambientes — `homolog.tuteladigital.com.br` e `www.tuteladigital.com.br` (produção). A extensão a produção, fora da sequência original do plano ("produção só como sprint futura, após período de observação"), foi confirmada pelo responsável do projeto como **deploy manual intencional, feito para resolver um problema que afetava produção** — não é uma aplicação acidental nem não autorizada. Nenhum `Content-Security-Policy` de enforcement (sem sufixo Report-Only) foi encontrado nas páginas estáticas de nenhum dos dois ambientes — o único header `Content-Security-Policy` de enforcement observado está nas respostas de `/api/diagnostico` e pertence à própria aplicação `tutela-api` (assinatura de middleware tipo Helmet — `x-download-options`, `x-permitted-cross-domain-policies`, `x-dns-prefetch-control`, `x-ratelimit-*` — política diferente da desenhada no Bloco B), pré-existente e não relacionado ao ARQ-102. **Ainda não fechar como CONCLUÍDO integralmente**: o critério de aceite original exige o header `Content-Security-Policy` de enforcement (modo bloqueante) em 100% das respostas — hoje só o Report-Only está ativo; falta o período de observação sem violações inesperadas e a decisão explícita de virar para modo bloqueante. |
| ADR relacionado | Nenhum (a criar em ARQ-701) — a política final é candidata a ADR curto quando for para modo bloqueante |
| Métrica de sucesso | 0 violações de CSP reportadas no console em auditoria manual das 37 páginas |
| Observações | Recomenda-se modo `Content-Security-Policy-Report-Only` antes de aplicar em modo bloqueante. Escopo confirmado como puramente infraestrutural (fora deste repositório): ARQ-108 confirmou, via `nginx -T` na fonte, que CSP está ausente tanto em produção quanto em homologação, e a mudança é feita no Nginx dos servidores, não em código de aplicação — a única exceção seria a alternativa `<meta http-equiv>`, se a via Nginx não for escolhida. Evidência original: `docs/ambientes-e-deploy.md`, seção "Auditoria externa (ARQ-108)". **Sprint 38 (2026-08-05) — Bloco A (mapa de origens, evidência por grep + captura de rede via Playwright contra `npm run dev`)**: `script-src` precisa de `www.googletagmanager.com` (gtag.js, `consent.js:69`), `www.google.com` (reCAPTCHA v2 checkbox, `api.js`, `diagnostico.js:213` — confirmado v2 pelo padrão `grecaptcha.getResponse()`, não v3) e `www.gstatic.com` (bundle `recaptcha__en.js`, confirmado carregado também no frame de topo, não só dentro do iframe do widget); `'unsafe-inline'` é necessário em `script-src` e `style-src` — o site tem ~18 páginas com blocos `<script>` inline distintos (não é só o loader de fontes de `header.html:8`) e `style=""`/`<style>` inline em várias páginas (`governo.html`, `termos-de-uso.html`, partial `ativos-digitais-pillar-styles.html`), sem templating/build para gerar nonce ou lista de hashes por página; blocos `<script type="application/ld+json">` são isentos de `script-src` pela spec (HTML não os trata como conteúdo executável) e não contam para essa necessidade. `font-src 'self'` confirmado suficiente — todas as fontes são locais (`/assets/fonts/*.woff2`, vendorizadas desde a Sprint 30); a única requisição a `fonts.gstatic.com` observada na captura de rede vem de dentro do iframe do próprio reCAPTCHA (`www.google.com/recaptcha/api2/anchor`), documento de origem do Google, não sujeito ao CSP da nossa página. `img-src` precisa de `data:` — 4 arquivos CSS usam `background: url("data:image/svg+xml,...")` (`diagnostico.css`, `seguranca.css`, `assets-digital.css`, `pages-consolidated.css`); nenhum CDN de imagem externo encontrado. `connect-src` precisa de `'self'` (`/api/diagnostico`, mesma origem via proxy Nginx, confirmado em `ambientes-e-deploy.md`), `https://www.google-analytics.com` (beacon padrão do GA4, `/g/collect`) e **também** `https://www.google.com` — achado da captura ao vivo: o gtag.js do GA4 dispara um segundo beacon para `www.google.com/g/collect` (parâmetro `gaf=1`, sinal de recurso de anúncios) diretamente no frame de topo, não é um efeito do reCAPTCHA. `frame-src` precisa de `https://www.google.com` (iframe `recaptcha/api2/anchor`, confirmado via `page.frames()`). Nenhum `<form>` nativo existe no site (busca `grep '<form'` vazia) — `form-action 'self'` é aditivo/seguro. Nenhum `<iframe>` de terceiro além do reCAPTCHA. Política completa (Bloco B) e evidência de rede detalhada apresentadas ao usuário na sessão; snippet Nginx exato preparado para `tutela-dev` (container `tutela_v2_nginx`) mas não aplicado — ver `docs/ambientes-e-deploy.md`, seção "CSP Report-Only (ARQ-102)". **Achado de processo, não de CSP**: `docs/ambientes-e-deploy.md` (linhas 159/266, pré-existentes) já documentava que nenhuma sessão anterior do Claude Code teve acesso SSH aos servidores — todo trabalho de infraestrutura em sprints passadas (ARQ-108, ARQ-505, renovação de certificado) seguiu o padrão "usuário roda o comando no servidor e cola o resultado de volta". Confirmado novamente nesta sprint por teste direto (`ssh tutela-dev`/`tutela-web`: hostname não resolve neste ambiente). O prompt desta sprint presumia acesso SSH "já demonstrado" — presunção incorreta, sinalizada ao usuário antes de prosseguir para o Bloco C. **Atualização (14/08/2026)**: reconfirmado via `nginx -T`/`curl -I` diretamente nos servidores que `Content-Security-Policy-Report-Only` segue ativo, sem mudança de conteúdo, em produção e homologação; nenhum `Content-Security-Policy` de enforcement foi introduzido. `docs/architecture/09-security.md` — que até então descrevia a existência desses headers como "necessita validação" — foi atualizado para refletir esse status confirmado. Segue faltando o período de observação sem violações e a decisão explícita de virar para modo bloqueante, critério de aceite original deste item, inalterado. |
...
| Status | CONCLUÍDO (confirmado ativo em produção desde a auditoria de 2026-07-24/ARQ-108; status deste item corrigido para CONCLUÍDO em 14/08/2026 — o header já estava presente na tabela comparativa daquela auditoria, mas o campo Status deste item específico não havia sido atualizado até agora) |
| ADR relacionado | Nenhum (a criar em ARQ-701) |
| Métrica de sucesso | Header presente em 100% das respostas; `max-age` ≥ 6 meses após período de validação — **atingido**: `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload` confirmado ativo em produção via `nginx -T`/`curl -I` diretamente no servidor. |
| Observações | Não ativar `preload` na primeira iteração — **fato observado, não revertido**: a configuração confirmada em produção já inclui `preload`, indo além da recomendação original deste item. Reverter está fora do escopo desta sprint de documentação (auditoria, não alteração de infraestrutura); registrado aqui apenas como fato. Homologação segue sem HSTS (não coberto pelo escopo original de "produção" deste item; ver tabela comparativa em `docs/ambientes-e-deploy.md`). |
...
| Estimativa | P |
| Responsável | Infraestrutura |
| Status | CONCLUÍDO (implantado e validado em produção e homologação em 14/08/2026 — confirmado ausente na auditoria original de 2026-07-24/ARQ-108, ver histórico abaixo) |
| ADR relacionado | Nenhum (a criar em ARQ-701) |
| Métrica de sucesso | Header presente em 100% das respostas — **atingido**: `Permissions-Policy: camera=(), microphone=(), geolocation=()` confirmado ativo em produção e homologação via `nginx -T`/`curl -I` diretamente nos servidores, 14/08/2026. |
| Observações | Escopo confirmado como puramente infraestrutural (fora deste repositório): a auditoria de 2026-07-24 (ARQ-108) havia confirmado, via `nginx -T` na fonte, que `Permissions-Policy` estava ausente tanto em produção quanto em homologação. **Atualização (14/08/2026)**: o header foi implantado no Nginx dos dois servidores e validado nesta sprint, restringindo câmera, microfone e geolocalização, conforme o critério de aceite original — mudança feita diretamente na configuração do Nginx (fora deste repositório), sem alteração de código de aplicação. Evidência: `docs/ambientes-e-deploy.md`, seção "Auditoria externa (ARQ-108)" e sua atualização de 14/08/2026. |
...
| Arquivos afetados | Novo `public/partials/cookie-banner.html`; `public/assets/js/consent.js` (novo); `public/assets/css/components/cookie-banner.css` (novo); `public/partials/scripts.html`; `public/partials/header.html`; `public/assets/js/i18n.js`; `public/assets/lang/pt.json`, `en.json`, `es.json`; remoção do snippet inline de GA das 16 páginas que o tinham; `tests/cookie-consent.spec.ts` (novo, 16 testes); `tests/visual-contrast.spec.ts`, `visual-design-tokens.spec.ts`, `visual-radius-shadow.spec.ts` (ajustados para não quebrar com o banner) |
...
| Status | IMPLEMENTAÇÃO TÉCNICA CONCLUÍDA (Sprint 37, 2026-08-04) — **não fechar como CONCLUÍDO**: texto do banner foi redigido e aguarda aprovação do responsável pelo projeto (decisão de conteúdo, mesmo padrão das Sprints 8/10/15/26) e, depois disso, revisão jurídica formal (LGPD/GDPR) antes de considerar o item formalmente encerrado |
| ADR relacionado | Nenhum (a criar em ARQ-701) |
| Métrica de sucesso | 0 disparos de `gtag`/GTM antes do consentimento explícito, verificado via inspeção de rede — **atingido tecnicamente e coberto por teste automatizado** (`tests/cookie-consent.spec.ts`, intercepta requisições a `googletagmanager.com`/`google-analytics.com`) |
| Observações | **Sprint 37 (2026-08-04)** — Bloco A: GA4 (`G-KXVB267PYJ`) deixou de ser um snippet inline duplicado em 16 páginas (achado da Sprint 36) e passou a ser carregado por `public/assets/js/consent.js`, centralizado via `partials/scripts.html` — cobre as 37 páginas reais do site (exceto `insights/ativos-digitais/o-que-sao-ativos-digitais/`, stub de redirect puro sem `scripts.html`, exceção intencional documentada). Inconsistência pré-existente corrigida pela centralização: só 3 das 16 páginas com GA aplicavam o `linker` de cross-domain para `app.tuteladigital.com.br` (o CTA do header, presente em 100% das páginas, aponta para lá) — o script único agora aplica esse config de forma consistente em todas as páginas. Bloco B: banner opt-in estrito (categoria única, sem diferenciação geográfica), `public/partials/cookie-banner.html`, incluído via SSI dentro de `partials/header.html` logo após o skip-link (sem alterar a ordem de foco: skip-link continua sendo o primeiro elemento focável, coberto por teste). Persistência: `localStorage` (`cookieConsent: {status, timestamp}`), validade de 12 meses; banner reaparece automaticamente quando expira ou não existe. Estilo reaproveita tokens existentes (`--color-surface-brand`, `--color-text-inverse`, `--color-accent*`, `--radius-8`, `--shadow-black-lg`, breakpoint `640px` já aprovado) — nenhum token novo introduzido (ARQ-301/débito #6 permanece eliminado). Dois bugs pegos e corrigidos durante validação manual em navegador antes dos testes automatizados: (1) `flex: 1 1 320px` no texto do banner virava `flex-basis` de *altura* (não largura) ao trocar para `flex-direction: column` no breakpoint mobile, fazendo o banner ocupar quase a tela inteira — corrigido para `flex: 1 1 auto` + `min-width`; (2) o botão flutuante de WhatsApp (`.tdf-wa`, `position: fixed`, `z-index: 9999`) sobrepunha o botão "Aceitar" do banner — corrigido ocultando `.tdf-wa` via classe `body.has-cookie-banner` enquanto o banner está visível. `i18n.js`: `applyInterfaceOnlyTranslations()` (usada em páginas legais quando idioma ≠ pt, que só traduzem chrome, não o corpo) passou a incluir `.cookie-banner [data-i18n]` — sem isso, o banner ficaria em português mesmo com o resto do chrome em EN/ES nessas páginas; validado manualmente e coberto por teste. Testes: `tests/cookie-consent.spec.ts` (16 testes) — sem requisição ao Google antes de interação (1 por cluster: home, ativos-digitais, legal, insights, todos passando), aceitar/recusar/persistência entre reloads, expiração de consentimento (>12 meses reabre o banner), skip-link preservado, tradução nos 3 idiomas, tradução em página legal não-pt, página de redirect puro sem banner/GA, e 2 páginas que antes não tinham GA nenhum (`/ativos-digitais/`, `/insights/ativos-digitais/marco-regulatorio/`) confirmando a nova cobertura respeitando o opt-in. Suítes de regressão visual (`visual-contrast`, `visual-design-tokens`, `visual-radius-shadow`) precisaram de um `beforeEach` pré-registrando recusa de consentimento (via `addInitScript`, sem rede real ao Google) para não quebrar com o banner cobrindo parte do viewport em screenshots — comportamento do banner em si só é testado em `cookie-consent.spec.ts`. `npm test` completo: mesmo baseline pré-existente de 2 falhas de regressão visual não relacionadas (`Footer: recorte` e `Dropdown de navegação aberto`, `visual-contrast.spec.ts`) — confirmado via `git stash` que ambas já falhavam na branch `homolog` antes desta sprint, fora do escopo aqui. Texto do banner (pt/en/es) redigido e apresentado ao responsável pelo projeto para aprovação nesta mesma sessão — commit local só depois da aprovação, sem `git push` sem confirmação adicional (regra permanente do projeto). Recomendação explícita: revisão jurídica formal (LGPD/GDPR) antes de fechar este item como CONCLUÍDO — esta sprint entrega a implementação técnica, não uma determinação de conformidade. |
...
| Estimativa | P |
| Responsável | Infraestrutura |
| Status | CONCLUÍDO (Sprint 5, 2026-07-24; reconfirmado e atualizado com achados adicionais em 14/08/2026 — ver Observações. Item **não reaberto**: já estava CONCLUÍDO, esta é uma atualização factual, não uma reexecução da auditoria original) |
| ADR relacionado | Nenhum (a criar em ARQ-701) |
...
### ARQ-308 — Fechar lacuna de cobertura visual das páginas de insights afetadas por `ARQ-307`

| Campo | Valor |
|---|---|
| Objetivo | Fechar o risco residual registrado nas Observações de `ARQ-307`: as páginas de artigo de `/insights/ativos-digitais/*` e `/insights/prova-digital/*` que carregam `assets-digital.css` (mesmas 2 regras de espaçamento reduzidas em `ARQ-307`) não tinham nenhuma baseline de regressão visual dedicada, nem validação visual manual completa. |
| Descrição | `ARQ-307` (Sprint 31) documentou "9 páginas" extras nessa condição, mas a contagem estava desatualizada: `insights/prova-digital/ia-custodia-qualificada/` (criada em 2026-06-23, antes da Sprint 31) tem o mesmo `body.assets-page.assets-pillar-page` e carrega o mesmo `assets-digital.css`, mas não foi incluída na contagem original — confirmado por grep (`class="assets-page assets-pillar-page"` + `assets-digital.css`) nesta sprint. O número real é **10**, não 9. Todas as 10 foram capturadas em screenshot de página inteira (viewport 1440×900) e tiveram o padding do `.hero`/`main > section` confirmado via `getComputedStyle` — nenhuma regressão visual encontrada; o fix de `ARQ-307` se aplicou uniformemente aos 2 padrões de template já documentados (hero "cheio", 88px, 5 páginas incluindo `ia-custodia-qualificada`; hero "cartão", 0px por regra mais específica, 5 páginas), com `main > section` em 32px em todas. |
| Origem | Risco residual explicitamente registrado nas Observações de `ARQ-307` ("sem cobertura de teste automatizado... validação manual recomendada"), retomado como ponto de partida desta sprint por instrução do responsável pelo projeto |
| Documento | `06-design-system.md` |
| Item da dívida técnica | N/A — fechamento de lacuna de teste, não ajuste de densidade visual |
| Arquivos afetados | `tests/visual-contrast.spec.ts` (novo `test.describe`, 10 testes + 10 baselines novas); nenhum arquivo de produto alterado |
| Dependências (depende de) | ARQ-307 |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | As 10 páginas de artigo que carregam `assets-digital.css` com baseline de regressão visual de página inteira; nenhuma delas com espaçamento incorreto |
| Critérios de Regressão | `npm test` sem novas falhas além das 2 pré-existentes conhecidas |
| Impacto | Baixo (cobertura de teste; nenhuma correção de produto necessária — as 10 páginas já estavam corretas) |
| Risco | Baixo |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Frontend + QA |
| Status | CONCLUÍDO (Sprint 32, 2026-07-31) |
| ADR relacionado | Nenhum |
| Métrica de sucesso | 10/10 páginas com baseline visual dedicada — **atingido**: `insights-<slug>-full.png` para cada uma, geradas e confirmadas estáveis (2ª execução sem `--update-snapshots`, 10/10 passando sem diff) |
| Observações | **Contagem corrigida**: `ARQ-307` citava 9 páginas; o inventário real (grep por `class="assets-page assets-pillar-page"` + carregamento de `assets-digital.css`) encontra 10 — a diferença é `ia-custodia-qualificada`, ausente da varredura original provavelmente por ter sido adicionada ao repositório pouco antes da Sprint 31 sem repetição do grep. Achado incidental, sem impacto (a página já recebeu o fix de `ARQ-307` corretamente, só faltava contagem/cobertura). **Validação por medição, não só screenshot**: `getComputedStyle` confirmou `.hero { padding-top: 88px }` nas 5 páginas de hero "cheio" (`compliance-lgpd`, `custodia-ativos-digitais`, `marco-regulatorio`, `sucessao-digital`, `ia-custodia-qualificada`) e `.hero { padding-top: 0px }` (regra mais específica, comportamento documentado em `ARQ-307`) nas 5 de hero "cartão" (`cadeia-custodia-prova-digital`, `hash-criptografico-temporalidade`, `integridade-tecnica-admissibilidade`, `producao-antecipada-prova-digital`, `prova-digital-processo-civil-brasileiro`); `main > section` em 32px/32px (top/bottom) nas 10, incluindo `cta-section` com seu próprio valor mais específico (64px/56px, não tocado por `ARQ-307`, comportamento esperado). `npm test`: 101/103 antes; 111/113 depois (101 + 10 novos), mesmas 2 falhas pré-existentes conhecidas (`visual-contrast.spec.ts` footer/dropdown-header, pixel-diff não relacionado). |

### ARQ-309 — Reduzir espaçamento excessivo de `h2` em `.whitepaper-container` e tokenizar `margin-bottom` genérico (`assets-digital.css` + `pages-consolidated.css`)

| Campo | Valor |
|---|---|
| Objetivo | Reduzir o espaço excessivo antes de cada `h2` dentro de `.whitepaper-container` nas páginas que carregam `assets-digital.css`, por decisão de densidade visual do responsável pelo projeto (aprovação explícita: "executar bloco B"). |
| Descrição | `body.assets-page .whitepaper-container h2` (`assets-digital.css:168-171`) tinha `margin: 3.4rem 0 1rem` + `padding-top: 2rem` — **86,4px** de espaço antes de cada `h2`, valor literal e não tokenizado, idêntico ao padrão que `ARQ-306` já havia corrigido em `partials/ativos-digitais-pillar-styles.html` (Grupo A, pt/en/es). `assets-digital.css` nunca recebeu a mesma migração — resíduo de template não migrado, mesma categoria de achado de `ARQ-306`/`ARQ-307`. Afeta 38 headings em 8 páginas: `ativos-digitais/heranca-digital/` (2), `ativos-digitais/prova-digital-judicial/` (2), e 6 artigos de insights com padrão de hero "cartão" — `cadeia-custodia-prova-digital` (8), `hash-criptografico-temporalidade` (5), `integridade-tecnica-admissibilidade` (5), `producao-antecipada-prova-digital` (6), `prova-digital-processo-civil-brasileiro` (10). Os 5 artigos com hero "cheio" e as 5 páginas de pt/en/es não usam essa regra (confirmado por contagem de `.whitepaper-container h2` = 0 nessas páginas — usam outro container). |
| Origem | Achado da auditoria site-wide de espaçamento da Sprint 32 (Bloco B), apresentado com medição e proposta de valor (idêntica à de `ARQ-306`) antes da aplicação; aprovado pelo responsável pelo projeto na mesma sprint |
| Documento | `06-design-system.md` |
| Item da dívida técnica | N/A — ajuste de densidade visual, mesmo padrão de `ARQ-306`/`ARQ-307` |
| Arquivos afetados | `public/assets/css/assets-digital.css` (1 regra: `.whitepaper-container h2` margin-top/padding-top); `public/assets/css/pages/pages-consolidated.css` (1 regra: `.whitepaper-container` margin-bottom, achado #2 — ver Observações); `?v=` de `assets-digital.css` de 9→10 em 15 arquivos HTML consumidores (`tests/support/asset-versions.json` regenerado) |
| Dependências (depende de) | ARQ-306 (mesmo padrão de correção, precedente de valor) |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | Espaço antes de cada `h2` de `.whitepaper-container` reduzido de 86,4px para 56px nas 8 páginas afetadas, sem quebra de layout; `margin-bottom` de `.whitepaper-container`/`.legal-section-wrapper` reduzido de 80px para 64px (`--space-xl`) nas 5 páginas afetadas (achado #2), sem quebra de layout |
| Critérios de Regressão | `npm test` sem novas falhas além das 2 pré-existentes conhecidas; `tests/cache-busting.spec.ts` passando |
| Impacto | Baixo-médio (ajuste estético, 8 páginas) |
| Risco | Baixo (1 regra, sem sobreposição de combinador de irmão como em `ARQ-306` — confirmado por grep que `assets-digital.css` não tem regra concorrente de maior especificidade para este seletor) |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Frontend |
| Status | CONCLUÍDO (Sprint 32, 2026-07-31) |
| ADR relacionado | Nenhum (mudança pontual de CSS) |
| Métrica de sucesso | (1) `margin-top`/`padding-top` de `.whitepaper-container h2` 54,4px+32px→40px+16px (86,4px→56px), confirmado via `getComputedStyle` nas 8 páginas afetadas, incluindo as 2 (`heranca-digital`, `prova-digital-judicial`) sem baseline visual dedicada (validadas por screenshot manual). (2) `margin-bottom` de `.whitepaper-container`/`.legal-section-wrapper` 80px→64px, confirmado via `getComputedStyle` nas 5 páginas afetadas (`pt/ativos-digitais`, `en/digital-assets`, `es/activos-digitales`, `heranca-digital`, `prova-digital-judicial`) |
| Observações | **Dois achados do Bloco B, dois valores de destino distintos, ambos aprovados explicitamente pelo responsável pelo projeto antes da aplicação** (não órfãos — CSS ativo e visível, decisão de densidade em ambos os casos). **Achado #1** (`assets-digital.css`, `.whitepaper-container h2`): `margin-top: var(--space-lg)` (40px), `padding-top: var(--space-sm)` (16px) — reaproveita o valor já decidido em `ARQ-306` para o mesmo padrão visual em outro arquivo, não um novo valor. Sem o problema de especificidade de `ARQ-306`: confirmado por grep que `assets-digital.css` não tem regra de combinador de irmão adjacente (`> * + h2`) concorrente para este seletor. **Achado #2** (`pages-consolidated.css:460`, `.whitepaper-container { margin: 0 auto 5rem auto }`): regra genérica sem escopo de página (nenhum `body.*` prefixando o seletor), incluída via `@import` em `main.css` — carregada por praticamente todas as páginas do site, mas só produz efeito visível onde nada mais específico sobrescreve `margin-bottom` (5 páginas: as 3 de idioma do cluster pt/en/es, via `.legal-section-wrapper` que herda o mesmo `margin-bottom` por cascata dentro do wrapper, e `heranca-digital`/`prova-digital-judicial`, via `.whitepaper-container` diretamente). Não teve `?v=` para bumpar — `main.css`/`pages-consolidated.css` são carregados sem query string de cache-busting em nenhuma página (confirmado: `grep` não encontra `main.css?v=` em nenhum HTML), fora do escopo do sistema de versionamento de `ARQ-502`; `tests/support/generate-asset-versions.js` recalculou o hash automaticamente, `tests/cache-busting.spec.ts` permaneceu verde sem nenhuma mudança adicional. Valor de destino (64px, `--space-xl`) escolhido pelo responsável pelo projeto entre 3 alternativas apresentadas (64px/`--space-xl`, 96px/`--space-2xl`, ou tokenizar mantendo 80px) — não presumido pela IA. **Validação por diff visual, ambos os achados**: 5 das 10 baselines de `ARQ-308` (hero "cartão") e as 3 baselines full-page de `ARQ-301` (pt/en/es) mostraram diff esperado (cascata vertical uniforme, sem distorção horizontal/cor/quebra) — revisado manualmente, baselines regeneradas (`--update-snapshots`, escopo restrito aos testes afetados). `heranca-digital`/`prova-digital-judicial` (sem baseline dedicada) verificadas por screenshot manual em cada rodada — sem quebra. `npm test`: 111/113 antes e depois de cada achado (mesmas 2 falhas pré-existentes). |

### ARQ-307 — Reduzir espaçamento vertical excessivo do hero e das seções do Grupo B (`assets-digital.css`)

| Campo | Valor |
|---|---|
| Objetivo | Reduzir a distância entre o header e o topo do hero, e o espaçamento acima/abaixo da primeira seção, nas páginas que usam `assets-digital.css` (`.hero`/`main > section`), a pedido do responsável pelo projeto (captura de tela de `/ativos-digitais/estrutura-juridica/`). |
| Descrição | `body.assets-page.assets-pillar-page .hero` usava `padding-top: var(--ad-space-xl, 5rem)` → `--ad-space-xl` resolve para `--space-feature` (7rem/112px) — o mesmo valor "antigo" que o `ARQ-305` já havia reduzido para `--space-section-hero` (5.5rem/88px) em 4 outros templates; este arquivo nunca recebeu a mesma migração. `body.assets-page.assets-pillar-page main > section` usava `padding: 4rem 0` (64px) literal, não tokenizado — soma 128px entre duas seções consecutivas (bottom de uma + top da próxima). |
| Origem | Ajuste de densidade visual solicitado pelo responsável pelo projeto (captura de tela de `/ativos-digitais/estrutura-juridica/`), com proposta de valores apresentada e aprovada antes da aplicação — não corresponde a nenhum item de `12-technical-debt.md` |
| Documento | `06-design-system.md` |
| Item da dívida técnica | N/A — ajuste de densidade visual |
| Arquivos afetados | `public/assets/css/assets-digital.css` (2 regras: `.hero` padding-top, `main > section` padding); `?v=` bumped em 15 arquivos HTML que referenciam o CSS (`tests/support/asset-versions.json` regenerado) |
| Dependências (depende de) | Nenhuma |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | Hero mais próximo do header e espaçamento de seção reduzido nas páginas que usam essas 2 regras, sem quebra de layout |
| Critérios de Regressão | `npm test` sem novas falhas além das 2 pré-existentes conhecidas; `tests/cache-busting.spec.ts` passando (manifesto sincronizado com o bump de `?v=`) |
| Impacto | Baixo-médio (ajuste estético, escopo maior que o inicialmente estimado — ver Observações) |
| Risco | Baixo (2 regras, únicas consumidoras dos tokens `--ad-space-xl`/`--ad-space-lg`, confirmado por grep antes da mudança; valor de `main > section` era literal, sem outro consumidor) |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Frontend |
| Status | CONCLUÍDO (Sprint 31, 2026-07-31) |
| ADR relacionado | Nenhum (mudança pontual de CSS) |
| Métrica de sucesso | Hero padding-top 112px→88px e `main > section` padding 64/64px→32/32px confirmados via `getComputedStyle` nas 5 páginas do Grupo B (`/ativos-digitais/` + 4 subpáginas) |
| Observações | **Escopo real maior que o proposto inicialmente**: a proposta original considerava só as 5 páginas do Grupo B (`/ativos-digitais/*`), mas `assets-digital.css` também é carregado por 9 páginas de artigo em `/insights/ativos-digitais/*` e `/insights/prova-digital/*`, que **também** carregam `class="assets-page assets-pillar-page"` no `<body>` — confirmado só depois de bumpar o `?v=` e localizar todos os consumidores do arquivo. Como as 2 regras editadas usam exatamente essas classes (não uma classe própria do Grupo B), a mudança se aplica às 14 páginas, não só 5. Verificado visualmente (screenshot) em 4 dessas páginas extras (`hash-criptografico-temporalidade`, `compliance-lgpd`, `cadeia-custodia-prova-digital`, `marco-regulatorio`) — 2 padrões distintos coexistem: páginas com hero "cartão" (`.page-header-inner`, herdado do partial de pilar, `.hero` externo com `padding: 0` por regra mais específica — não afetadas pela mudança de `.hero`) e páginas com hero "cheio" no estilo Grupo B (afetadas normalmente pelas 2 regras). Em ambos os casos, a redução de `main > section` se aplica de forma consistente e sem sobreposição/quebra visual (confirmado por screenshot, sem regressão). **Sem cobertura de teste automatizado** para essas 9 páginas extras (nenhum spec de regressão visual as cobre individualmente) — validação manual recomendada em homologação antes do merge para `main`, sinalizado como risco residual. **Achado, não corrigido**: os overrides de `h2` em `@media (max-width: 768px)` do partial de pilar (`ARQ-306`) tinham um problema de especificidade pré-existente (regra de combinador de irmão fora da media query sempre vencia) — não relacionado a este item, mas do mesmo padrão de "regra mais específica escondida"; aqui não houve o mesmo problema porque `.hero` (3 classes) e `main > section` (2 classes + `>` ) não têm regra concorrente de maior especificidade no mesmo arquivo. **Validação**: `npm test` 101/103 antes e depois; `tests/cache-busting.spec.ts` (que havia falhado por falta de bump de `?v=`) voltou a passar após bump em 15 arquivos + regeneração do manifesto; baselines `ativos-digitais-full` (`visual-contrast.spec.ts`) e `radius-shadow-ativos-digitais-full` (`visual-radius-shadow.spec.ts`) regeneradas após revisão manual do diff (mesmo padrão de deslocamento vertical em cascata do `ARQ-306`); **cuidado processual registrado**: o primeiro `--update-snapshots` rodou no arquivo de spec inteiro e regenerou por engano os 2 baselines pré-existentes não relacionados (`footer`, `dropdown-open-header`, falhas conhecidas de pixel-diff) — revertidos via `git checkout` antes do commit, mantendo o baseline documentado de 101/103 (não 103/103), para não mascarar o problema conhecido sem investigação própria. |

### ARQ-306 — Reduzir espaçamento vertical excessivo do cluster Ativos Digitais (páginas pt/en/es)

| Campo | Valor |
|---|---|
| Objetivo | Reduzir o espaçamento vertical percebido como excessivo nas 3 páginas "pillar" do cluster Ativos Digitais (`/pt/ativos-digitais/`, `/en/digital-assets/`, `/es/activos-digitales/`), por decisão de densidade visual do responsável pelo projeto. |
| Descrição | `partials/ativos-digitais-pillar-styles.html` (CSS inline incluído via SSI só nessas 3 páginas — ver ARQ-301) nunca foi migrado para a escala `--space-*` global (ARQ-301/303/305 não tocaram este padrão específico de espaçamento). Usava valores `rem` ad-hoc, dissociados da escala compartilhada: cada `h2` do `whitepaper-container` (11 ocorrências por página) tinha 86,4px de espaço antes do texto (`margin-top: 3.4rem`/`3.6rem` + `padding-top: 2rem`) — mais que o próprio `--space-section` (64px), token usado para padding padrão de seção inteira — e os blocos `stats-grid`/`grid-2`/`steps`/`checklist` usavam `margin-bottom: 2.75rem` (44px), acima do `--space-section-tight` (32px, token específico para "seções internas sem quebra de fundo", o caso exato desses blocos). Medição confirmou: 950px (≈16% da altura total da página) só no padrão repetido de `h2`. |
| Origem | Ajuste de densidade visual solicitado pelo responsável pelo projeto, com proposta de valores apresentada e aprovada antes da aplicação (mesma disciplina de decisão de conteúdo das Sprints 8/10/15/26) — não corresponde a nenhum dos itens numerados de `12-technical-debt.md` |
| Documento | `06-design-system.md` |
| Item da dívida técnica | N/A — ajuste de densidade visual, não item pré-existente da dívida técnica |
| Arquivos afetados | `public/partials/ativos-digitais-pillar-styles.html` (único arquivo funcional alterado) |
| Dependências (depende de) | Nenhuma |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | Espaçamento vertical das 3 páginas do cluster reduzido conforme valores aprovados, sem quebra de layout em desktop/mobile |
| Critérios de Regressão | `npm test` sem novas falhas além das 2 pré-existentes conhecidas; baselines granulares de `tests/visual-design-tokens.spec.ts` (ARQ-301) mostrando só deslocamento vertical em cascata, sem nenhuma outra distorção |
| Impacto | Baixo-médio (ajuste estético em 3 páginas, sem afetar funcionalidade) |
| Risco | Baixo (mudança de valores de espaçamento em CSS já isolado por partial exclusivo dessas 3 páginas; escopo confirmado por grep antes de qualquer mudança) |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Frontend |
| Status | CONCLUÍDO (Sprint 31, 2026-07-31) |
| ADR relacionado | Nenhum (mudança pontual de CSS, sem decisão de arquitetura) |
| Métrica de sucesso | Altura total de página reduzida em desktop e mobile nas 3 páginas — **atingido**: desktop 5968px→5618px (-350px, -5,9%), mobile 7887px→7580px (-307px, -3,9%), medido via `document.body.scrollHeight` antes/depois (`git stash`/`git stash pop` para comparação controlada no mesmo servidor). |
| Observações | **Escopo confirmado antes de qualquer mudança**: o menu "Ativos Digitais" tem 2 famílias de template distintas — Grupo A (as 3 páginas de idioma, este item, CSS via `ativos-digitais-pillar-styles.html`, tokens `--ad-pillar-*`) e Grupo B (`/ativos-digitais/` + 4 subpáginas, CSS via `assets-digital.css`, vocabulário de seção totalmente diferente — `pilares-section`/`desafio-section`/`muda-section`/`contexto-section` — nunca tokenizado por ARQ-301, fora do escopo deste item). **Proposta apresentada e aprovada antes da aplicação**: `h2` margin-top 3,4rem/3,6rem→`var(--space-lg)` (40px), padding-top 2rem→`var(--space-sm)` (16px); `stats-grid`/`grid-2`/`steps`/`checklist` margin-bottom 2,75rem→`var(--space-section-tight)` (32px); `h2` mobile margin-top/padding-top 2,6rem/1,5rem→1,75rem/0,75rem (valores literais, sem token exato disponível). **Achado durante a implementação**: a regra mais específica `.whitepaper-container > p/ul/ol/div + h2` (combinador de irmão adjacente) vencia a cascata para a maioria dos `h2` reais da página, não a regra base `.whitepaper-container h2` — as duas precisaram ser ajustadas em conjunto, senão a mudança não teria efeito visual prático. **Achado incidental, não corrigido**: o override mobile de `margin-top` (dentro de `@media (max-width: 768px)`) nunca teve efeito real mesmo antes desta sprint — a regra de combinador de irmão, fora de qualquer media query, sempre venceu por especificidade (2 classes + 2 elementos > 2 classes + 1 elemento). `padding-top` mobile não é afetado por essa regra e funciona normalmente. Corrigir exigiria reestruturar seletores (aumento de especificidade ou reorganização), fora do escopo pontual de ajuste de valores desta sprint — registrado para decisão futura. **Validação**: `npm test` 101/103 antes; após a mudança, 19 falhas (17 novas, todas as baselines granulares de `visual-design-tokens.spec.ts` para pt/en/es nos 6 blocos tocados — esperado); diffs revisados manualmente (`whitepaper-container`, `stats-grid`) confirmando deslocamento vertical puro em cascata, sem distorção horizontal, sem mudança de cor, sem quebra de card; baselines regeneradas (`npx playwright test tests/visual-design-tokens.spec.ts --update-snapshots`, 24/24 passando); `npm test` final: 101/103, de volta ao baseline conhecido. Screenshots "antes"/"depois" (desktop + mobile, 3 páginas) ficam fora do repositório (artefato de investigação, não de produto). |

### ARQ-305 — Remover compensação de `padding-top` redundante do header (header sticky, faixa vazia entre header e hero)

| Campo | Valor |
|---|---|
| Objetivo | Eliminar uma faixa vazia visível entre o `.header` e o início real do conteúdo do hero em 4 templates (home, como-funciona, seguranca, prova-digital-validade-juridica), reportada pelo responsável pelo projeto via captura de tela. |
| Descrição | `.main { padding-top: 60px; }` (`layout/layout.css`) é uma compensação herdada da época em que `.header` era `position: fixed; height: 70px` (commit anterior a `e107cc7`/`22a22d7`, 2026-03-27/30) — nessa época, o header saía do fluxo normal do documento e `.main` precisava de padding equivalente para não ficar coberto. Desde `e107cc7`/`22a22d7`, `.header` é `position: sticky`, que **permanece no fluxo normal** e já empurra `.main` para baixo sozinho — tornando esse padding-top redundante em qualquer página onde nada mais o zera. A maioria dos templates (legal/\*, insights/\*, clusters de idioma de ativos-digitais) recebeu, em refatorações posteriores não relacionadas, um modificador `.main--hero-top` que zera (ou reduz) esse padding como efeito colateral — mas 4 templates nunca receberam esse modificador, deixando os 60px redundantes visíveis como uma faixa vazia. O commit `575e23e` (2026-07-22, "padroniza ritmo vertical e contraste dos templates") reduziu o padding interno do hero desses mesmos 4 templates (7rem→5.5rem) sem tocar nessa causa raiz, tornando a faixa (já existente, porém antes disfarçada dentro do espaçamento maior do hero) visualmente destacada — provável gatilho da percepção do bug pelo usuário, embora a causa raiz seja anterior (~4 meses, migração fixed→sticky). |
| Origem | Bug reportado pelo responsável pelo projeto (captura de tela, faixa vazia acima do badge "Infraestrutura Probatória Digital" na home) — não corresponde a nenhum dos 16 itens numerados de `12-technical-debt.md` |
| Documento | `06-design-system.md` |
| Item da dívida técnica | N/A — bug de regressão de layout, não item pré-existente da dívida técnica |
| Arquivos afetados | `public/assets/css/layout/layout.css` (remove `padding-top: 60px` da regra base `.main`); `public/assets/css/pages/dark-editorial-shared.css` (remove `body.has-fixed-header .main { padding-top: 60px; }` — confirmado morto, nenhuma página usa a classe `has-fixed-header`, mesma causa raiz) |
| Dependências (depende de) | Nenhuma |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | Nenhuma faixa vazia visível entre `.header` e o início do hero nas 37 páginas reais do site; nenhuma página anteriormente correta (`gap: 0`) regride |
| Critérios de Regressão | `npm test` sem novas falhas além das 2 pré-existentes conhecidas (`footer`/`dropdown-open-header`, pixel-diff não relacionado); baselines visuais de página completa (`visual-contrast.spec.ts`, `visual-design-tokens.spec.ts`, `visual-radius-shadow.spec.ts`) mostrando só deslocamento vertical uniforme do conteúdo (altura de página reduzida), sem nenhuma outra distorção |
| Impacto | Médio (defeito visual em 4 páginas, incluindo a home) |
| Risco | Baixo (remoção de propriedade CSS única, sem lógica associada; superfície medida em todas as 37 páginas antes e depois) |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Frontend |
| Status | CONCLUÍDO (Sprint 30, 2026-07-31) |
| ADR relacionado | Nenhum (mudança pontual de CSS, sem decisão de arquitetura) |
| Métrica de sucesso | 0/37 páginas com faixa vazia visível entre header e hero — **atingido**, confirmado por script Playwright que mede `getBoundingClientRect` do `.header` e do primeiro filho renderizado de `.main`/`main` em todas as 37 rotas reais (derivadas de `git ls-files`, mesma lógica de `tests/sitemap-route-parity.spec.ts`), com screenshot de cada página antes e depois. |
| Observações | **Mapa completo (Bloco A, medição em `viewport 1440×900`, antes da correção)**: **afetadas** (gap = 60px, mesmo fundo escuro do `.app` acima e abaixo — faixa visível) — `/` (home), `/como-funciona/`, `/seguranca/`, `/prova-digital-validade-juridica/` (esta com fundo claro, faixa menos contrastante mas real). **Não afetadas visualmente, mas com o mesmo padding redundante mascarado por cor de fundo idêntica ao header** (gap = 76px, corrigido como efeito colateral da mesma remoção, sem risco pois nenhuma página dependia desses 76px para respiro visual) — `/pt/ativos-digitais/`, `/en/digital-assets/`, `/es/activos-digitales/`. **Não afetadas, `.main--hero-top` já zera via regra `body.legal-page .main.main--hero-top`** (gap = 0) — as 7 páginas de `legal/*`. **Não afetadas, regra própria com `!important` em `insights-pilar.css` (`padding-top: 18px`, não relacionada a esta causa raiz, deliberadamente não alterada)** — `/insights/`, `/insights/ativos-digitais/`, `/insights/prova-digital/` (gap = 18px, banner de aviso de idioma ocupa o espaço, sem faixa visível). **Não afetadas, override `!important` dedicado** — `/diagnostico/` (`.diagnostico-page .main { padding-top: 0 !important; }`). **Nunca afetadas, `<main>` sem `class="main"`** (a regra `.main` nunca se aplicou) — `/governo/`, `/empresas/`, `/pessoas/`, as 5 páginas do cluster `/ativos-digitais/*` e os 11 artigos individuais de `/insights/ativos-digitais/*`/`/insights/prova-digital/*`. **N/A, página de redirect sem header/hero** — `/insights/ativos-digitais/o-que-sao-ativos-digitais/` (stub `<meta http-equiv="refresh">`, sem `.header`/`.main`). Screenshots de todas as 37 páginas (antes e depois) e o script de medição ficam fora do repositório (artefato de investigação, não de produto). **Correção aplicada**: 2 arquivos, ambos remoção de propriedade/regra (nenhuma linha nova de CSS funcional) — `layout/layout.css` (`.main` perde `padding-top: 60px`, substituído por comentário explicando por que nenhum valor é necessário com header sticky) e `pages/dark-editorial-shared.css` (remove bloco morto `body.has-fixed-header .main`). **Achado relacionado, não corrigido nesta sprint por não ter efeito visual e por disciplina de escopo**: `body.legal-page .main { padding-top: 60px; /* compensação header fixo */ }` (`pages-consolidated.css:744`) carrega o mesmo comentário/raciocínio obsoleto, mas está sempre neutralizada por `.main--hero-top` nas páginas onde aparece — registrado aqui como possível limpeza futura de dívida técnica, não como bug. **Validação**: `npm test` 101/103 antes e depois (2 falhas pré-existentes conhecidas, não relacionadas — `visual-contrast.spec.ts` footer/dropdown-header, pixel-diff já registrado em sprints anteriores); baselines de página completa afetadas pela mudança de altura (`home-full`, `pt/en/es-full`, `radius-shadow-home-full`) regeneradas após revisão manual do diff pixel a pixel, confirmando deslocamento vertical uniforme do conteúdo sem nenhuma outra alteração. |
...
### ARQ-408 — Divergência de normalização de URL (`.html`/barra final) entre produção e homologação

| Campo | Valor |
|---|---|
| Objetivo | Registrar, sem corrigir, que a regra genérica de normalização de URL do Nginx não é idêntica entre os dois ambientes. |
| Descrição | Produção normaliza `.html`/URLs sem barra final para a forma canônica com barra final (`301` no Nginx do host: `.../$1/`). Homologação remove `.html` mas não força barra final (`301` no Nginx do container: `.../$1`, sem barra). Confirmado na config-fonte dos dois ambientes (ver tabela comparativa em `docs/ambientes-e-deploy.md`, seção "Nginx") desde a auditoria de 2026-07-24 (ARQ-108) e reconfirmado em 14/08/2026, sem mudança de comportamento. |
| Origem | Achado da auditoria de infraestrutura Nginx (ARQ-108) — tabela comparativa de 2026-07-24, formalizado como item próprio em 14/08/2026 para não sobrecarregar ainda mais um item de auditoria já concluído |
| Documento | `docs/ambientes-e-deploy.md` |
| Item da dívida técnica | Nenhum — divergência de ambiente, não item de `12-technical-debt.md` |
| Arquivos afetados | Nenhum arquivo deste repositório — configuração Nginx fora do repositório, nos dois servidores |
| Dependências (depende de) | ARQ-108 |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | Divergência documentada com evidência de config-fonte nos dois ambientes; decisão explícita registrada sobre unificar ou manter como está |
| Critérios de Regressão | Não aplicável (item de registro; nenhuma mudança de comportamento) |
| Impacto | Baixo — sem impacto funcional identificado (mesmo conteúdo final servido; `404` correto em ambos para rotas inexistentes, distinto do bug de ARQ-403) |
| Risco | Baixo |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Infraestrutura |
...
| ADR relacionado | Nenhum |
| Métrica de sucesso | Divergência registrada e rastreável; nenhuma métrica de correção, já que não há decisão de unificar tomada nesta sprint |
| Observações | Distinto do bug de ARQ-403 (5 URLs legadas retornando `404` em ambos os ambientes, causa raiz já documentada e não afetada por este item): aqui o comportamento de cada ambiente já é o esperado para as URLs que ele de fato resolve — só a forma final da URL (com ou sem barra) diverge entre eles. Não corrigido nesta sprint por instrução explícita do escopo (tarefa de documentação, não de alteração de infraestrutura) — decisão de unificar ou manter como está fica para sprint futura. |
...
### ARQ-508 — Corrigir falso positivo do gate `checklist-publicacao` em arquivos CSS com CRLF (`.gitattributes`)

| Campo | Valor |
|---|---|
| Objetivo | Eliminar um falso positivo do gate de CI `checklist-publicacao` (`git diff --check`, introduzido em `ARQ-504`): qualquer linha alterada em um arquivo CSS que usa terminador de linha CRLF falha o check com "trailing whitespace", mesmo sem nenhum problema real de formatação. |
| Descrição | `ARQ-504` registrou como risco residual que o job `checklist-publicacao` nunca tinha sido validado contra um PR real via GitHub Actions. Esse risco se materializou na PR #195 (Sprint 32, `ARQ-309`): a edição de uma linha em `pages-consolidated.css` (arquivo CRLF desde `ARQ-303`) falhou `git diff --check` com "trailing whitespace" na linha `margin: 0 auto var(--space-xl) auto;` — o conteúdo estava correto, mas o `\r` do CRLF é tratado como erro de whitespace por padrão pelo Git (`core.whitespace`), a menos que o atributo `cr-at-eol` esteja habilitado, o que o repositório nunca configurou. Sem correção, **qualquer edição futura** a um dos arquivos CRLF do repositório falharia esse gate, independente do conteúdo da mudança — bloqueio estrutural, não pontual. |
| Origem | Bloqueio real encontrado durante a publicação de `ARQ-309` (Sprint 32); risco residual de `ARQ-504` que se confirmou na primeira validação real do gate contra um PR de produção |
| Documento | `13-development-workflow.md` |
| Item da dívida técnica | N/A — correção de infraestrutura de CI, achado incidental fora do escopo original de `ARQ-309` |
| Arquivos afetados | `.gitattributes` (novo) |
| Dependências (depende de) | ARQ-504 (introduziu o gate), ARQ-303 (decisão original de preservar CRLF nesses arquivos) |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | `git diff --check` não falha por causa de CRLF em nenhum dos arquivos CSS que legitimamente usam esse terminador; continua falhando normalmente para whitespace problemático real (espaço em branco no fim de linha, marcador de conflito não resolvido) |
| Critérios de Regressão | Nenhum PR legítimo passa a falhar por falso positivo; nenhum PR com problema real de whitespace deixa de ser barrado |
| Impacto | Médio (bloqueava a publicação de qualquer mudança futura, por menor que fosse, aos 9 arquivos CSS afetados) |
| Risco | Baixo (`.gitattributes` não altera nenhum byte de conteúdo versionado, só a interpretação de whitespace pelas ferramentas Git) |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | DevOps |
| Status | CONCLUÍDO (Sprint 32, 2026-07-31) |
...
| Estimativa | M |
| Responsável | Frontend |
| Status | CONCLUÍDO (Sprint 9, 2026-07-24). **Sprint 39 (2026-08-14)**: `ArrowUp` no toggle passou a também abrir o dropdown e focar o **último** item do submenu (antes, só `ArrowDown` fazia isso, focando o primeiro) — simetria de padrão WAI-ARIA menu button nas duas direções. Único gap real identificado nesta revisão; Tab, Enter/Space, Esc, navegação entre itens do submenu aberto e `:focus-within` já estavam corretos desde a Sprint 9, sem alteração. |
| ADR relacionado | Nenhum (a criar em ARQ-701) |
...
### ARQ-608 — Corrigir contraste de `.section-overtitle` em seções claras de `institucional.html`

| Campo | Valor |
|---|---|
| Objetivo | Corrigir o rótulo "supertítulo" (`.section-overtitle`) das seções "Infraestrutura" e "Casos de uso" em `/legal/institucional.html`, quase invisível contra o fundo claro da página — reportado pelo responsável pelo projeto via captura de tela. |
| Descrição | `.section-overtitle` (`legal-shared.css`) usa `color: var(--primitive-green-200, #a8d9b8)` — um verde pastel desenhado para o fundo escuro do hero (`.page-header--legal`, gradiente `--primitive-green-950`→`--primitive-green-700`) e das seções `.section-dark` (`--primitive-green-950` sólido, com override próprio para `--primitive-green-500`). As 2 seções `.features` de `institucional.html` ("Infraestrutura"/"Camadas de Confiança", "Casos de uso"/"Finalidade da Infraestrutura") usam essa mesma classe sem `.section-dark`, herdando a cor clara sobre o fundo claro padrão da página (`rgb(242,247,245)`) — contraste medido de **1,46:1**, muito abaixo do mínimo WCAG AA (4,5:1 para texto normal). |
| Origem | Bug reportado pelo responsável pelo projeto (captura de tela, rótulo "INFRAESTRUTURA" quase ilegível) — item pendente desde a auditoria de legibilidade da Sprint 32 (Bloco D), que mediu tipografia/contraste do corpo de texto da página mas não cobriu este rótulo específico |
| Documento | `06-design-system.md` |
| Item da dívida técnica | N/A — bug de contraste, mesma categoria de `ARQ-604`/`ARQ-606` |
| Arquivos afetados | `public/assets/css/pages/institucional.css` (1 regra nova); `?v=` de `institucional.css` 6→7 (único arquivo consumidor, `tests/support/asset-versions.json` regenerado) |
| Dependências (depende de) | Nenhuma |
| Dependências (desbloqueia) | Nenhuma |
| Pré-requisitos | Nenhum |
| Critérios de Aceite | Contraste de `.section-overtitle` ≥ 4,5:1 nas 2 seções claras afetadas; hero e `.section-dark` (fundo escuro) inalterados |
| Critérios de Regressão | `npm test` sem novas falhas além das 2 pré-existentes conhecidas |
| Impacto | Médio (rótulo de seção praticamente ilegível, mas não bloqueia leitura do conteúdo principal) |
| Risco | Baixo (regra nova e específica, `.features:not(.section-dark) .section-overtitle`, sem tocar a regra base compartilhada com outras páginas) |
| Complexidade | Baixa |
| Estimativa | P |
| Responsável | Frontend |
| Status | CONCLUÍDO (Sprint 32, 2026-07-31) |
| ADR relacionado | Nenhum |
| Métrica de sucesso | Contraste 1,46:1 → **5,96:1** (`--primitive-green-700`, `#1b6b4d`), confirmado via `getComputedStyle` + cálculo de luminância relativa WCAG nas 2 seções afetadas |
| Observações | **Escopo confirmado por grep antes da mudança**: `.section-overtitle` é usado em só 2 páginas do repositório — `institucional.html` (2 das 3 ocorrências quebradas, a 3ª é o hero, correto) e `preservacao-probatoria-digital/index.html` (1 ocorrência, já dentro de `.features.section-dark`, contraste correto — não precisou de mudança). **Valor reaproveitado, não inventado**: `--primitive-green-700` já é a cor de destaque usada na mesma seção (borda esquerda de `.features-inner h2`), garantindo consistência visual em vez de introduzir uma 3ª cor de acento na página. **Escopo do seletor**: `.features:not(.section-dark) .section-overtitle`, adicionado em `institucional.css` (carregado só por esta página — confirmado por grep, zero risco para outras páginas), sem tocar a regra base em `legal-shared.css` (usada pelo hero desta página e por `preservacao-probatoria-digital`, ambos corretos e não relacionados a este bug). **Achado adjacente, não corrigido**: a auditoria de legibilidade do Bloco D da Sprint 32 mediu tipografia/contraste do corpo de texto (parágrafos, headings) e não cobriu rótulos "overtitle"/"eyebrow" — lacuna de escopo da auditoria original, fechada aqui via achado reportado diretamente pelo responsável pelo projeto, não por nova varredura sistemática de todos os pequenos elementos de UI da página (fora do escopo desta correção pontual). `npm test`: 111/113 antes e depois (mesmas 2 falhas pré-existentes); nenhuma baseline visual dedicada a `institucional.html` existia para atualizar. |
...
| Estimativa | M-G |
| Responsável | Infraestrutura |
| Status | CONCLUÍDO — Caminho B implementado e validado em homologação e produção (14/08/2026) |
...
| Métrica de sucesso | atingido nos dois ambientes — Tutela, Netcenter e Veritio negociando X25519MLKEM768 em produção, confirmado via teste externo através do hostname público |
| Observações | Diretiva aplicada em homologação: `ssl_ecdh_curve X25519MLKEM768:X25519:secp256r1;`, em `/opt/tutela-v2/nginx/default.conf` de `homolog.tuteladigital.com.br`, aplicada via `docker compose up -d --force-recreate nginx` (bind mount de arquivo único — `reload` sozinho não é suficiente, ver `docs/ambientes-e-deploy.md`). Em produção, o Caminho B (`oqs-provider`) foi escolhido e implementado (Caminhos A e C descartados por risco/complexidade desproporcionais — ver ADR-0004). A primeira tentativa de aplicar a diretiva em produção falhou com `handshake_failure`: o processo master do Nginx, ativo desde antes da mudança, nunca recarregou sua inicialização do OpenSSL — `systemctl reload nginx` não é suficiente após editar `openssl.cnf`, é necessário `systemctl restart nginx` para o novo provider ser carregado pelos workers. Detalhamento completo do diagnóstico no ADR-0004. Como a diretiva foi aplicada no nível `http{}` global do Nginx do host, Netcenter e Veritio (que compartilham o mesmo processo) passaram a negociar PQC automaticamente, sem trabalho adicional. |
